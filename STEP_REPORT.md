...
- **페이지 근거(`p.6`) → 슬라이드 리더 이동**은 여전히 미구현이다(1단계 `[X]` 항목).
  10단계에서 함께 처리하려 했으나, 그건 `parseConceptSource`가 페이지 locator를 앵커로
  해석하도록 **데이터 계약을 넓히는 일**이라 FIX 10(레이아웃)과 성격이 다르다.
  리더가 탭 전환에 살아남게 된 지금은 붙이기 쉬운 상태가 됐으므로, 별도 작업으로 남긴다.
